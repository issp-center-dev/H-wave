...
相互作用テーブルは、文書化された向き（``[rx] [ry] [rz] [alpha] [beta] ...``\ という行が軌道\ ``[alpha]``\ を元のセルに、\ ``[beta]``\ を\ :math:`\vec{r}`\ だけ並進したセルに置く、トランスファーのテーブルと同じ規約）で読み込まれ、H-wave 2.0.0 からの変更点は相互作用指定ファイルの節の注意書きに記載しています。
