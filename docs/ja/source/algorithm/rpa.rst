...
.. note::
   ...
   （スピン軌道相互作用など）、横チャネルは全スピン軌道空間で
   ドレスされます -- 縦チャネルと同じ ``general``\ スキームの求解
   です -- そのうえで :math:`\chi_{+-}(\mathbf{q})`\ はドレスされた
   テンソルからスピン反転ブロックとして抽出されます。したがって
   スピン混合のクロス項は完全に含まれ、警告は出力されません。
