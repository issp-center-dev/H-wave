...
.. note::
   ...
   **セクターと、バブルをバンド対角に制限しない理由。**

   :math:`{\cal H}_0` がある量子数（例えばスピン）を保存する場合、Green 関数は
   それについてブロック対角になり
   :math:`G^{(0)}_{\alpha\sigma,\beta\tau}
   =\delta_{\sigma\tau}G^{(0)\sigma}_{\alpha\beta}` と書けます。このとき上の
   ...
   バブルを近似しているのではなく、チャネルを取り出しているだけであり、両者は
   混ざらないためです（その条件は後述のブロック対角化の節で述べます）。

   一方、固有基底の **バンド** 添字を制限するのは別の操作です。これが
   :math:`\sigma=\sigma'` を選ぶことと一致するのは、バンドがセクターそのもので
   ある場合、すなわち単一軌道でスピンに依存しない :math:`{\cal H}_0` のように
   2つのバンドが2つのスピン状態である場合に限られます。バンドが重ね合わせに
   なった途端（軌道の混成、あるいはスピンと軌道の混成）、捨てられる項は密度成分
   にも寄与するようになり、この制限は制御されない近似になります。H-wave は
   バンド添字を制限しません。

   逆にセクターの **内部** では行列構造を保つ必要があります。これを潰すこと、
