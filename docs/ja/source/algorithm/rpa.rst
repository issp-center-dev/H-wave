...
あります）。また、この\ :math:`q`\ 非依存性検査が対象とする相互作用の種類に
ついては、\ ``SubShape``\ による折り畳みでスーパーセル内に収まるサイト間ペアは
セル内軌道ペアとなり、表現可能として受理されます。ただし\ ``PairHop``\ は例外で、
その局所性は元の（折り畳み前の）宣言で判定されます。そのため、オフサイトの
``PairHop``\ は\ ``SubShape``\ による折り畳みでセル内軌道ペアになる場合でも
破棄され、ボンド分解横方向チャネルでは拒否されます（下記の警告を参照）。
縦方向（\ ``ring``\ ）チャネルは影響を受けません。

.. warning::

   オフサイトの\ ``PairHop``\ は **表現されません** 。これは未実装ではなく
   **恒久的な制限** です。オフサイトの\ ``PairHop``\ の横方向寄与は、横方向
   チャネルが扱う粒子・正孔チャネル空間
   :math:`c^\dagger_{i a \uparrow} c_{j b \downarrow}`\ の外にある
   ペアホッピング型の双線形であり、表現するには双線形基底の拡張が必要だから
   です。通常の計算では、これらのエントリは相互作用の読み込み時にこの検査より
   前に破棄されるため、拒否も反映もされません（破棄した宣言を列挙する警告が
   ログに出力されます）。RPA 計算でオフサイトの\ ``PairHop``\ に依存しないで
   ください。実験的なボンド分解横方向チャネル
   （\ ``transverse_bond_channels = true``\ ）では、オフサイトの
   ``PairHop``\ の宣言は代わりに **エラーとして拒否** されます。これは、
   不完全な横方向の結果が黙って生成されることを防ぐためです。オンサイトの
   ``PairHop``\ はいずれの場合も完全にサポートされます。
   また、対角（同一軌道）の PairHop は密度項
   :math:`2P\, n_\uparrow n_\downarrow`\ を意味しますが、読み込み時の係数は
   :math:`2P`\ ではなく\ :math:`P`\ として（縦・横チャネルで一貫して）
...
となります。これは対基底の密度スロット上の行列（それ以外のスロットではゼロ）で
...
ついては近似であり、H-wave 2.0.0 以前のリリースで得られた結果を再現するために
残されています。
...
.. note::
   ...
   もともと直接\ :math:`V`\ の2次を厳密に含んでいました）。このオプション
   （``longitudinal_bond_channels = true``\ ）を有効にした場合でも
...
ring は縦方向（スピン・電荷）チャネルを、移行運動量\ :math:`q`\ のみに
依存する頂点で足し上げます。オフサイト相互作用\ :math:`V_{ab}(R)`\ に
ついて、この頂点は Hartree 部分\ :math:`V_{ab}(q)`\ を含みますが、
交換（Fock）交差は含みません。交換交差の粒子・正孔対は1サイト上では
なくボンド\ :math:`(i, i+R)`\ の上にあり、2つのフェルミオン運動量に
依存するため、\ :math:`q`\ のみでは表現できないからです。
``longitudinal_bond_channels = true``\ （\ ``calc_type = "ring"``\ 、
``calc_scheme = "general"``\ 、スピンフリー系）では、静的な縦方向チャネルを
代わりに Eliashberg のボンドチャネル拡張と同じボンド拡張対基底の上で
足し上げます。対の添字は\ ``(m, l_1, l_2)``\ を走り、\ :math:`m`\ は
ボンドチャネル（オンサイト\ :math:`R = 0`\ と、宣言された全ての
オフサイト変位を\ :math:`R \to -R`\ について閉じたもの）、裸のバブルは
:math:`\Omega = 0`\ における
:math:`\bar\chi_{m l_1 l_2;\, m' l_3 l_4}(q) = -\frac{T}{N}\sum_k
e^{ik\cdot(R_m - R_{m'})} G_{l_1 l_3}(k+q)\, G_{l_4 l_2}(k)`\ 、
頂点は\ :math:`m = m' = 0`\ ブロックに標準の ring の対空間行列を置き、
さらにボンド対角\ :math:`(m, l_1, l_2) = (m', l_3, l_4)`\ （\ :math:`m \neq 0`\ ）
にオフサイト項の交換交差\ :math:`w_t\, \mathrm{Re}\, V^{(t)}_{l_1 l_2}(R_m)`\ を
置いたものです。ここで\ :math:`w_t`\ はオンサイトの交換スロットと同じ
...
:math:`(m = 0, m' = 0)`\ ブロックが\ ``chiq``\ ファイルの
``longitudinal_bond_*``\ キーに書き出されます。ファイル内の配列\ ``chiq``\ は
上書きされず、標準の ring の結果のままです。オフサイト係数が全てゼロなら縮約ブロックは
標準の ring の静的スピン・電荷チャネルと一致し、非零なら交換交差の分だけ
異なります。ボンドブロックはオフサイトの\ ``CoulombInter``\ ・
``Hund``\ ・\ ``Ising``\ （同一軌道・軌道間ボンド）について結合の1次
まで厳密対角化と照合され、Hartree のみの頂点では同じデータを再現
できないことも確認されています。

このバージョンの制限: オフサイト係数は実数のみ。オフサイトの
``Exchange``\ と\ ``PairHop``\ の宣言は拒否されます（ボンド基底での
...
:math:`UV`\ 交差項を持ちません。（このオプション（``longitudinal_bond_channels = true``\ ）を
有効にした場合でも、軌道間のオフサイトボンドではボンド分解ブロックに小さな2次のずれが
残ります。軌道対角のボンドは厳密です。）このような計算の通常の
