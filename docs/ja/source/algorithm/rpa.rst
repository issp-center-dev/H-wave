...
各双一次形式は2つの独立なバンド添字を持ちます。H-wave は相互作用を一貫して
軌道基底で扱い、対角化は Green 関数の構成にのみ用います。
...
.. note::
   ...
   **バブルをバンド対角に制限しない理由。**
   バンド分解のうち :math:`\gamma=\gamma'` のみを残すのは自然な簡約に見え、
   実際ある特別な場合には無害です。それは、バンドのラベル自身が保存量である
   場合です。単一軌道でスピンに依存しない :math:`{\cal H}_0` なら、2つの
   バンドは2つのスピン状態であり、捨てられる :math:`\gamma\neq\gamma'` の項は
   ちょうどスピン反転（横）チャネルにあたるので、密度応答は影響を受けません。
   しかしバンドが重ね合わせになった途端（軌道の混成、あるいはスピンと軌道の
   混成）、これらの項は密度成分にも寄与するようになり、制限すると結果が
   壊れます。

   この制限が導入される動機になりがちな落とし穴は、実は別のものです。
   :math:`G` の行列構造を潰してしまうこと、例えばバブルを作る前に
   :math:`G_{\uparrow}+G_{\downarrow}` というスカラーに置き換えることです。
   すると交差項 :math:`G_{\uparrow}G_{\downarrow}` が、本来属さない密度応答に
   混入します。スピンが縮退していれば結果はちょうど2倍になります。上記のように
   :math:`G` を行列のまま保てば、何も捨てずにこれを避けられます。各伝播関数の
   線が自身の軌道・スピン添字を持ち、チャネルはその添字構造によって自動的に
   分離されるためです。
