...
``chiq`` は各々がスピンを担う対を添字とするため、対がスピン非対角となる
スロットを持ちます。 **これらのスロットは横感受率ではなく、既定の**
``calc_type = "ring"`` **では計算されません。** スピン自由度がスピンフリー
あるいはスピン対角なバブルの膨張によって導入される場合---すなわち
``enable_spin_orbital`` を使わないすべての計算---膨張は同スピンのスロットしか
作らないため、スピン非対角のスロットは恒等的にゼロになります。感受率配列の
...
.. note::
   ...
   対称性が成り立つ範囲ではこれは ``chiq_pm`` を厳密に再現するので、
   この目的にラダーは不要です。

   SU(2) が破れた途端にこの関係は成り立ちません。1軌道正方格子
   （ :math:`U = 4` 、 :math:`T = 0.5` 、 :math:`4\times 4` ）で
   ゼーマン分裂 :math:`h = 0.35` を加えると、この推定は横方向応答の
   ピーク値を約 2 パーセント過大評価し、正しい値を返すのはラダーだけです。
   磁場、磁気秩序変数、スピン軌道相互作用がある場合、横方向チャネルは
   計算する必要があります。
