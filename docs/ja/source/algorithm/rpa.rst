...
（ :math:`\gamma_1=\gamma_2` ）のみを残すと帯間の寄与が落ち、縮退部分空間では
基底の取り方に依存する結果になります（詳しくは既約感受率の箇所を参照）。
そのような制限は行いません。H-wave は相互作用を一貫して軌道基底で扱い、
対角化は Green 関数の構成にのみ用います。
...
異なるバンドに乗ることができます。これらを同一バンドに縛ると帯間の粒子・正孔遷移が
落ちますが、 :math:`{\bf q}=0` から離れるとこの寄与が応答の大半を占めます。
6サイト2軌道鎖では、 :math:`{\bf q}=0` の密度応答は変わりません（そこでは
全電子数演算子が厳密にバンド対角であるため）が、 :math:`q=\pi/3` で 29%、
:math:`2\pi/3` で 71% を失い、ゾーン境界ではバンド対角のバブルはほぼゼロに
なります（0.788 に対して 0.004）。 :math:`k` と :math:`k+q` の状態が同一バンド内で
ほぼ直交するためです。さらに縮退部分空間ではこの制限は基底に依存し、スピン縮退
したバンドではスピン反転チャネルが失われて SU(2) 対称性を破ります。したがって
そのような制限は行いません。
