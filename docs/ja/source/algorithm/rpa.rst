...
スピン非対角となるスロットを常に持ちます。 ``"squashed"`` もバブルが膨張で
得られる場合には持ち、そのときの配列はスピン軸を明示的に含みます。
``"reduced"`` は密度対の成分のみを格納するため、そのようなスロットはそもそも
存在しません。真にスピンフルな計算における ``"squashed"`` も同様で、この場合は
reduced の配列形式に帰着します。
