...
完全な Green 関数から作られるバブルは、独立な :math:`\gamma` と
:math:`\gamma'` についての二重のバンド和を含みます。これを対角
:math:`\gamma=\gamma'` に制限すること、すなわち応答をバンド密度のみから
組み立ててバンド間双一次形式を捨てることが厳密になるのは、バンド添字自体が
:math:`{\cal H}_0` の保存量である場合に限られます。バンド間遷移を禁止できるのは
保存則だけだからです。そうでない場合、この制限は次のように振る舞います。

* 充満したバンドがフェルミ準位から十分離れており、それを横切るバンドとの混成が
  弱い場合、この制限は制御された近似になります。誤差は混成 :math:`v` と準位間隔
  :math:`\Delta` について :math:`(v/\Delta)^2` の程度です。
* フェルミ準位付近で軌道が強く混成している場合、バンド間項はバンド内項と同程度に
  なり、誤差は大きくなります。
* 完全にギャップが開いた絶縁体では、各バンドは完全に充満しているか完全に空である
  ためバンド内項が恒等的にゼロになり、応答はすべてバンド間が担います。この制限は
  ゼロを返します。ギャップを広げても改善しません。応答の大きさを決めるのは
  ギャップではなくバンド間行列要素だからです。
* バンドが接する点では、この制限はそもそも一意に定まりません。縮退部分空間の
  どの正規直交基底も正当な固有基底であり、対角部分はその任意の選択に依存する
  一方、全和は依存しません。

なお、後述の **セクター** を選ぶ操作はこれとは別物で、厳密性を保ちます。ここで
問題になるのはバンド対角制限だけです。

以上より、H-wave は両方のバンド和を保持します。これは実装としても単純です。
バンド添字は現れた直後に和を取って消えるため、バンド順序の追跡も、ブリルアン
ゾーンにわたる連続性の条件も、縮退の特別扱いも、どこにも必要ありません。
相互作用は一貫して軌道基底で扱い、対角化は Green 関数の構成にのみ用います。
...
これはバンド添字を制限する操作とは別物です。両者が一致するのは、バンドが
セクターそのものである場合、すなわち単一軌道でスピンに依存しない
:math:`{\cal H}_0` のように2つのバンドが2つのスピン状態である場合に限られます。
両者が異なる場合でもセクターの選択は厳密なままですが、バンド対角制限はそうでは
ありません。
