...
電荷チャネル（ :math:`W_c` ）を用いて計算されます。 :math:`U` と
:math:`V` は宣言ファイルの対称化された読み（逆向きボンドの相手
:math:`(R, a, b) \leftrightarrow (-R, b, a)` との平均）として入り、
パッケージ内の他の全経路と一致します。両方向を含む実数の宣言はビット単位で不変に読まれます（エルミート共役
で閉じた複素対は、不活性な虚部が実係数に折り畳まれます）
（バージョン 2.0 以降、エルミート共役の相手を欠く片側宣言は読み込み時に拒否
されるため、ここに到達するのは閉じたファイルのみです。以前の簡易モード
は片側ファイルを生の位相で読んでいました）:
...
.. note::
   ...
   ます。また general スキーム（``myo``）のファイルでは、CoulombInter・
   Hund・Ising・Exchange のいずれかに非対称なオンサイト軌道間結合
   （ :math:`X_{ab} \neq X_{ba}` ）が宣言されている場合も、バージョン無
   しファイルは拒否されます。相互作用が頂点に入る軌道の向きと、そのよう
   な宣言の対称化された読み方は、過去のビルドと現在とで異なっており、バ
   ージョン無しファイルはどのセマンティクスで生成されたかを記録していな
   いためです。PairHop は単純な転置ではなく
   エルミート共役の対に対して検査されます（2 つの宣言はその対なので向き自体
   は変わっていませんが、エルミート共役で閉じていないオンサイト宣言は、そ
   の共役平均による読み方がバージョン刻印と同時に導入されたため、同様に
   拒否されます）。PairLift は対象外です（粒子・正孔頂点への寄与が厳密に
   ゼロであり、向きも対称化された読み方も影響し得ません）。影響を受けない条件を規則ごとに述べると、
   転置検査される 4 種の相互作用については転置対称な宣言
   （ :math:`X_{ab} = X_{ba}` 、物理的に通常の入力すべて）、PairHop に
   ついてはエルミート共役で閉じた宣言（ :math:`P_{ba} = P_{ab}^{*}` 、
   物理的に妥当な形であり、転置対称である必要はありません）です。
   reduced スキームのファイルはこの向きに依存しません。
...
松原振動数の和を ``Nmat`` 点の有限グリッド上で実行することで、
二粒子グリーン関数が得られます:
...
.. note::
   ...
   **松原 tail 補正（バージョン 2.0 以降）。** 打ち切り和は厳密な二粒子グリーン
   関数の主要な正の単位行列 tail を取りこぼします（ :math:`O(1/N_{\rm mat})`
   の誤差）。被和関数の厳密な
   高周波 tail は
   :math:`\delta_{\alpha\gamma}\delta_{\beta\delta}/\omega_n^2` です
   （:math:`G` の :math:`1/i\omega_n` 係数は固有基底の完全性により単位
   行列で、これは FLEX のドレストグリーン関数でも同じです）。デフォルト
   ではソルバーがこのモデルを周波数窓内で差し引き、その厳密な全和
   :math:`T \sum_{n \in \mathbb{Z}} 1/\omega_n^2 = \beta/4` を解析的に
   加えます。これはギャップ空間上の単位行列の正の倍数を加えることに相当
   し、カーネルのスペクトルを実にする :math:`G^{(2)}` の半正定値性を実用上
   回復します（高次の打ち切り残差により微小な負固有値が残る場合があり、
   その場合ソルバーが警告します）。補正なしでは、小さい ``Nmat`` の多軌道
   計算が偽の複素固有値を報告することがあり、対称性の破れと誤読される
   おそれがあります。補正導入前の結果を
   再現するには ``[eliashberg]`` セクションで ``g2_tail = false`` を設定
   してください。主要な :math:`1/\omega_n^2` 項を除去し、対称グリッド上で
   奇数次逆べきが相殺するため、除外和の誤差は :math:`O(N_{\rm mat}^{-3})`
   でスケールします（通常の整数べき高周波展開を仮定）。この補正は漸近的で、
   保持される最大周波数が関連するエネルギー・自己エネルギースケールを超え
   ない場合は過補正になり得ます。窓端でグリーン関数が
   :math:`I/(i\omega_n)` から要素毎の最大偏差で 0.5 を超えて外れている
   場合、ソルバーは警告を出します。また、計算された :math:`G^{(2)}` が有意に負の固有値を持つ場合も
   ``Nmat`` を指す警告を出します（これはカーネルの実スペクトル性に関する
   構造的診断であり、誤差の推定ではありません）。動的（周波数分解）
   ソルバーはこの周波数和を取らないため影響を受けません。
