...
.. note::
   ...
   Hermite 対に対して検査されます（2 つの宣言は Hermite 対なので向き自体
   は変わっていませんが、Hermite 閉包になっていないオンサイト宣言は、そ
   の共役平均による読み方がバージョン刻印と同時に導入されたため、同様に
   拒否されます）。PairLift は対象外です（粒子・正孔頂点への寄与が厳密に
   ゼロであり、向きも対称化された読み方も影響し得ません）。対称な宣言（物理的に通常
   の入力すべて）は影響を受けず、reduced スキームのファイルはこの向きに
   依存しません。
