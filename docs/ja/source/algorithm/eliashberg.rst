...
.. note::
   ...
   ます。また general スキーム（``myo``）のファイルでは、設定された相互
   作用に非対称なオンサイト軌道間結合（ :math:`X_{ab} \neq X_{ba}` ）が
   宣言されている場合も、バージョン無しファイルは拒否されます。相互作用
   が頂点に入る軌道の向きと、そのような宣言の対称化された読み方は、いず
   れもバージョン刻印の導入以前に変更されており、古いファイルは現在の頂
   点とどちらか（または両方）が食い違う可能性があるためです。対称な宣言
   （物理的に通常の入力すべて）は影響を受けず、reduced スキームのファイ
   ルはこの向きに依存しません。
