...
.. note::
   ...
   なります。前者は厳密対角化により、longitudinal（スピン・電荷）チャネルに
   :math:`q` のみで表現できる頂点成分がないことが確認されているためです
   （局所 bilinear で書ける内容は transverse チャネルにあり、この spin-free
   経路には現れません。longitudinal に残る非局所（bond 分解）の交換交差は
   この経路では扱えません）。後者は局所対の形が存在しないためです。
   オンサイトの\ ``Exchange``
