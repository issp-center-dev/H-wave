...
.. warning::
   ...
   ``sigma.npz`` を種にしないでください。そのファイルは軌道非対角成分が誤って
   おり、SCFループは同じ誤った解に収束します。先に種を再生成してください
...
- ``chi_orbital_layout``: 軌道ペア添字の **順序**\ 。``"acbd"``\ 、すなわち行
  添字が第1ペア、列添字が第2ペアで、``chi0[a,c,b,d]`` に対応します。両スキーム
  ともこの順序で書き出すため、両方にこのマーカーが付きます。頂点と形状を選ぶ
  ``chi_convention`` とは役割が異なります。このマーカーは、修正前のgeneralパスが
...
.. note::
   ...
   ``chiq_s``/``chiq_c`` は転置されており、読み込み時に拒否されます。
...
.. note::
   ...
   .. warning::
      ...
      **``calc_scheme = "general"`` の結果が変わります。** 前リリースまでの
      general パスは、有効相互作用を構築する際に軌道ペア添字の余分な転置を
      行っていました。この転置により自己エネルギーが軌道添字について転置
      されるため、``sigma.npz`` および そこから導かれる量（占有数、
      エネルギー、Eliashberg固有値）が **軌道非対角成分について誤って
      いました**。1軌道の計算は影響を受けません（転置が恒等写像になるため）。
      多軌道の計算は、Green関数が軌道非対角の重みを持つ場合に影響を受け
      ます。密度成分のみの相互作用（``CoulombIntra`` のみ）では、general と
      reduced スキームの自己エネルギーが本来あるべき通り機械精度で一致する
      ようになりました。

      同じ転置により、保存される ``chiq_s``/``chiq_c`` も他の箇所で用いる
      ``[a,c,b,d]`` の順序ではなくMYOの軌道ペア順序で書き出されていました。
      このため ``hwave_sc`` の ``chi0q_mode = "flex"`` が、同一の物理に対して
      ``"load"`` と食い違う結果を与えていました。現在は ``[a,c,b,d]`` で
      書き出し、明示的な ``chi_orbital_layout = "acbd"`` マーカーを付与します。
      マーカーを持たない ``"myo"`` タグ付きファイルは修正前の出力なので、
      黙って誤読せず、再生成を促すメッセージとともに読み込みを拒否します。
      ``chi0q.npz`` は影響を受けません（出力時点で既に転置し戻されていました）。

      **再生成が必要なもの。** 修正前の ``calc_scheme = "general"`` 計算の
      多軌道出力すべて。``sigma.npz`` と ``green.npz`` （軌道非対角成分が
      誤り）、および ``chiq_s``/``chiq_c`` （転置されている）です。1軌道の
      計算は影響を受けず、``calc_scheme = "reduced"`` および ``"squashed"``
      も影響を受けません。
