...
また\ ``calc_scheme = "reduced"``\ スキームでは、
スピン/電荷バーテックスを相互作用の密度--密度成分で構成します。
バージョン 2.0 以降、``reduced``\ はデフォルトではありません。デフォルトは
``calc_scheme = "auto"``\ であり、``reduced``\ は H-wave 1.0.x の挙動を
再現するための明示的なオプトアウト（文書化された近似であり、構成時に一度
警告が出力されます）という位置づけです。``auto``\ は\ ``Exchange``/``PairHop``
がある場合、自動的に\ ``general``\ を選択します---これらは密度--密度成分の頂点を
**一切持たない** ため\ ``reduced``\ では近似すらできず、明示的な\ ``reduced``\ で
これらを与えると\ ``ValueError``\ となり、``calc_scheme = "general"``\ への
切り替えが案内されます（以前の開発版では警告付きで受理され、相互作用は無音で
効果ゼロになっていました）。``auto``\ は同様に、``CoulombInter``\ 、
``Hund``\ 、``Ising``\ 、または集約された\ ``Coulomb``\ 相互作用が指定
されている場合にも自動的に\ ``general``\ を選択します。これらは
...
H-wave 2.0.0 以前のリリースで得られた結果を再現するには
...
スキームのアーカイブや、本キーの導入前に出力されたアーカイブの場合は情報
メッセージを出します）。H-wave 2.0.0 以前のリリースが出力したアーカイブには
カーネルの記録が無いため、出るのは
...
強結合の計算は磁気的・電荷的な不安定性に近づくことがあります。Stoner
因子がおよそ 0.99 を超える、あるいはログの\ ``guards:``\ 行が報告する
ボンドの条件数の最小値（``cond_min``\ ）が
``[mode.param] longitudinal_bond_cond_tol``\ の1桁以内に入る、といった
状況です。この領域では自己無撞着計算が硬く（stiff）なり、不動点が複数
存在し得ます。以下の運用指針に従ってください。なお、この節で挙げる
キーはすべて\ ``[mode.param]``\ テーブルのものです。
...
- **線形混合を小さな刻みで使う。** ``mixing_scheme = "linear"``\ と
  ``Mix <= 0.03``\ を用います。刻みが大きいと不安定領域へ行き過ぎ、
  ドレスされたバーテックスが発散的に大きくなってガードが実行を
  停止します。
- **温度を小刻みに下げてウォームスタートする。** 温度を降順にたどり、
  各計算は1つ前のわずかに高い温度で収束した自己エネルギーから開始
...
- ``[mode.param]``\ で ``flex_guard_policy = "warn"`` **を設定するのは
  一時的な逸脱を通すときだけにする。** 等時刻密度の対称性
  （``flex_hf_density_tol``\ ）やボンドの条件数
  （``longitudinal_bond_cond_tol``\ ）が許容値に違反した反復を、実行を
  停止せずに継続させる設定です。厳密に特異な連立方程式や非有限な値が
  生じた場合は、いずれの設定でも実行は停止します。この設定が許容するのは
  特異に近い分母であって、特異な分母ではありません。最終状態の検査はこの
  設定によらず行われるため、最終状態の等時刻密度の偏差が
  ``flex_hf_density_tol``\ を超える場合も、最後の写像がボンドのガードの
  違反をこの設定で通り抜けていた場合も、実行は拒否されます（後者の拒否は
  最後の写像の条件数の最小値を報告します）。通過させた違反の件数は出力の
  ``flex_guard_violations``\ に記録されます。これは反復の回数ではなく
  違反 **事象** の件数で、密度については1反復につき1件、ボンドのガードに
  ついては該当する（チャネル, 振動数, :math:`q`\ 点）ごとに1件を数える
  ため、1回の反復が複数件を寄与することがあります。

反復ごとに出力される\ ``guards:``\ 行を読み、違反が減衰していること
（密度の偏差が\ ``flex_hf_density_tol``\ を下回り、\ ``cond_min``\ が
``longitudinal_bond_cond_tol``\ を上回るところまで戻り、そのまま留まる
こと）を確認してください。
...
この行は、等時刻密度のエルミート対称性の相対 Frobenius 偏差を
``flex_hf_density_tol``\ と並べて示し、\ ``longitudinal_bond_channels =
true``\ の場合はさらに、その反復の写像におけるスピン・電荷 RPA 分母の
条件数の最小値を\ ``longitudinal_bond_cond_tol``\ と並べて示します。

掃引では自動実行の側はデフォルトの\ ``flex_guard_policy = "refuse"``
のままにし、\ ``"warn"``\ は原因を確認した個別の点にのみ使ってください。
後処理スクリプトでは、収束した結果を採用する前に
``flex_guard_violations``\ が\ ``0``\ であることを確認してください。
件数は事象単位で数えるため、ボンドのガードでは硬い反復が1回あるだけで
多数の件数になり得ます。実際に影響を受けた反復の数は\ ``guards:``\ 行で
確認してください。

許容値そのもの（``flex_hf_density_tol``\ ・
``longitudinal_bond_cond_tol``\ ）は、意図的に硬い領域を調べるための
つまみであり、ガードを黙らせるためのものではありません。下限を
引き下げれば、増幅された丸め誤差に支配された数値のまま計算が
続いてしまいます。したがって、最終状態で拒否されたときの対処は
``flex_hf_density_tol``\ を引き上げることでも
``longitudinal_bond_cond_tol``\ を引き下げることでもありません。
``Mix``\ をより小さくし、温度の刻みをより細かくしてください。
...
（記録が無い場合、すなわち reduced スキームのアーカイブや、本キーの導入前
（H-wave 2.0.0 以前）に出力されたアーカイブの場合は INFO レベルの情報
メッセージを出します）。そのような計算が収束しない場合は
...
デバイス上に置かれたままで、バブルもそこで組み立てられるため、バブルの
組み立てに関して1回のマップでホストへ渡るのはその結果（チャネル対ごとに
1ブロック）だけです。ドレッシングと自己エネルギーの転送は従来どおり
それぞれのデータ（有効相互作用の振動数バッチ、続いて縮約と静的スライス）を
やり取りします。ボンド配列自体はホストメモリに置かれたまま振動数
...
``longitudinal_bond_nb``\ が記録されます。GPU 上の\ ``"all"``\ はもはや
従来ほどのコストがかかりません（ガードの条件数バウンドをデバイス上で
計算し、バッチ最小値になり得るブロックのみをホストで厳密に分解する
ため、出力は全ブロック分解の場合と完全に一致したまま、コストはおよそ
半減します）。そのため本番運用でも\ ``"all"``\ を既定のまま使うのが
妥当です。\ ``"static"``\ は縮約された診断のままであり、1 反復あたり
依然として約3倍高速なので、\ ``longitudinal_bond_guard_freqs = "all"``\
で一度検査した後に\ ``"static"``\ に切り替えるという従来の助言は、
スループットを重視する場合には引き続き有効です。
オンサイト\ ``U``\ と最近接\ ``V``\ を
...
<subsec:eliashberg_bond_outputs>`\ を、ボンド固有のキーは
:ref:`設定ファイルリファレンスの eliashberg 節
<eliashberg_bond_dynamic_config>`\ を、ソルバー全般の設定は
:ref:`動的振動数の節 <sc_dynamic_frequency>`\ を参照してください。
...
書き出されるものであり、多軌道モデルではディスク上で数十 GB に
なります。対形成カーネルは
...
ここでの GB は\ :math:`10^9`\ バイトを意味します。一方、上限を指定する
キー\ ``bond_memory_cap_gb``\ は2進の GiB（:math:`1024^3`\ バイト。
約7%大きい単位）です。上の数値からこのキーを設定する際は、その差を
考慮してください。
...
.. note::
   ...
   ``gap_dynamic.npz``\ をこの固定の名前で書き出します
   （``[eliashberg] output_gap`` / ``output_eigenvalue``\ で名前を
   変更できるのはテキストファイルの\ ``gap.dat`` / ``eigenvalue.dat``
   だけです）。singlet と triplet のチャネルは異なる2つの
   ``path_to_output``\ ディレクトリに出力してください。さもないと
   2回目の実行が1回目のアーカイブを上書きします。
...
.. note::
   ...
   :math:`F(0^+)`\ としてではありません。振動数について偶の対振幅に
   対しては両者は一致します（その偶数\ :math:`l`\ の IR 係数は消える
   ためです）。以前の処方はこれに加えて奇振動数成分を偶セクターへ
   写し、逆向きには作用しなかったため、振動数パリティに関して
   ブロック三角であり、スペクトルは同じでした。したがってパリティ
   射影を行った計算の固有値も固有値ソルバーの固有値も変わりませんが、
   固有ベクトルと射影なしの反復は影響を受けていました。この修正が
   取り除くのは、その偽のパリティの漏れ（およびそれに伴う射影の
   無効化——triplet の計算では singlet セクターの値に収束し得ました
   ——と、奇パリティチャネルのギャップ関数への偶パリティ成分の混入）
   です。同じ修正は
...
- ``flex_second_order`` / ``flex_second_order_schema``:
  ...
  および本キーの導入前（H-wave 2.0.0 以前）に出力されたアーカイブには含まれません。
  読み込み側がこれらを
...
.. list-table::
   ...
   * - ``ir_wmax``
     ...
     - IR基底の実周波数バンド幅（ハミルトニアンと同じエネルギー単位）。
       ...
       自動推定値は\ ``3*(max|eps - mu| + 相互作用)``\ （化学ポテンシャルを
       基準にしたスペクトル半幅＋最大相互作用スケール）で、動的 Eliashberg
       ソルバと同一です。これにより FLEX から Eliashberg へ渡す際に同じ基底に
       揃います。（以前は\ ``3*(2*max|eps| + 相互作用)``\ で、オンサイトの
       エネルギーオフセットを二重に数えていました。``ir_wmax``\ を明示指定した
       実行は影響を受けません。）
   * - ``sigma_init_on_error``
     - str
     - "warn"
   ...
   * - ``flex_second_order``
     ...
     - ``calc_scheme = "general"``\ のみ。有効相互作用の2次カーネルを
       ...
       Takimoto-Hotta-Ueda の式を用いて H-wave 2.0.0 以前のリリースの結果を
       再現します。
...
.. note::
   ...
   **ring の頂点には** Hartree（密度）部分 :math:`V(q)`\ としてのみ入り、
   ...
   ``PairHop``\ は局所対の形が存在しません。FLEX で計算する際はこれらの
   項を相互作用ファイルから除いてください。RPA ソルバー用に用意した
   相互作用（RPA はこれらを読み込みます）をそのまま流用することはできません。
   オンサイトの\ ``Exchange``
