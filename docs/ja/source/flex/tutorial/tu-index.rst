...
.. note::
   ...
   多軌道の ``sigma.npz`` は軌道非対角成分が誤っており、再生成が必要です。
   ``sigma_init`` の種として、あるいは ``hwave_sc`` の入力として使い回して
   いるファイルも対象です。
...
.. note::
   ...
   .. warning::
      ...
      **``calc_scheme = "general"`` の結果が変わります。** 前リリースまでの
      ...
      保存される ``chiq_s``/``chiq_c`` は、オンサイト相互作用パラメータが
      軌道添字について対称である限り（``U'[a,b] = U'[b,a]``、
      ``J[a,b] = J[b,a]``、``Exchange``/``PairHop``/``Ising`` も同様。これが
      物理的な場合です） **変わりません**。修正前後の式はこの場合に代数的に
      同一であり、差は行列積とチャネルの線形解の浮動小数点誤差のみです
      （条件の良い回帰テスト用データで相対10⁻¹⁵程度。``1 ∓ χ⁰U`` が特異に
      近い場合、すなわちRPA不安定点近傍ではこれより大きくなります）。
      相互作用ファイルが非対称な軌道行列を与えている場合は感受率も変化
      しますが、新しい値の方が整合的です（RPAソルバーや ``hwave_sc`` と
      同じ軌道ペア添字の順序で相互作用行列を適用します）。ただし共通なのは
      添字の順序のみで、general パスは ``C(ab,ab)`` の電荷頂点についてMYOの
      値を意図的に用いており、この点は ``hwave_sc`` と異なります。なお
      H-waveは現在、相互作用ファイルのこの対称性を検査していません。

      本修正以前に ``calc_scheme = "general"`` で得た多軌道の結果は再計算
      してください。``calc_scheme = "reduced"`` および ``"squashed"`` は
      影響を受けません。
