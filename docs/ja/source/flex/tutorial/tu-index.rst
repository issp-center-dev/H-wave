...
``longitudinal_bond_cond_tol``\ の1桁以内に入る、といった状況です。この
領域では自己無撞着計算が硬く（stiff）なり、不動点が複数存在し得ます。
以下の運用指針に従ってください。
...
- ``flex_guard_policy = "warn"`` **は一時的な逸脱を通すためだけに
  使う。** 密度の対称性やボンドの条件数が許容値に違反した反復を、実行を
  停止せずに継続させる設定です。計算後に\ ``guards:``\ 行を読み、違反が
  減衰していること（密度の偏差が\ ``flex_hf_density_tol``\ を下回り、
  ``cond_min``\ が\ ``longitudinal_bond_cond_tol``\ を上回るところまで
  戻り、そのまま留まること）を確認してください。最終状態の検査はこの
  設定によらず行われるため、最後まで違反が続く計算は最後の写像の条件数の
  最小値とともに拒否されます。通過させた違反の件数は出力の
