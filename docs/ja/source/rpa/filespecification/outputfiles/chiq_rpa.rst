...
``chiq`` は縦方向（ring）の感受率です。添字の対がスピン非対角となるスロットを持ちますが、これらは横感受率では **ありません** 。また既定の ``calc_type = ring`` では **計算されません** 。 ``enable_spin_orbital`` を使わないすべての計算では、スピン自由度はスピンフリーあるいはスピン対角なバブルの膨張によって導入され、膨張は同スピンのスロットしか作らないため、残りは恒等的にゼロのままです。このゼロを横方向応答の計算結果として読まないでください。横方向チャネルの求め方は :ref:`アルゴリズム<rpa_which_array>` を参照してください。
