...
（``"LM"``\ ＝素の\ ``arnoldi``\ の絶対値最大、``"LR"``\ ＝利用者指定の
``spectral_shift``\ による実部最大、``"LR_retry"``\ ＝``"LM"``\ の集合に正の
チャネル固有値が無かったときの実部最大を求める自動再求解、
...
``# eigenvalue_selection: <LM|LR|LR_retry|shift-invert|iteration>``\ の行が
