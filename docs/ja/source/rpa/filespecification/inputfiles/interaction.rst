...
.. note::
   ...
   **2.0.0 との互換性：**\ H-wave 2.0.0 までは、平均場ソルバー（``UHFk``\ 、
   ...
   変わりません。\ ``calc_type = "ring"``\ と
   ``longitudinal_bond_channels = true``\ を指定した RPA/FLEX 計算も、実数の
   軌道間オフサイトボンドでは結果が変わります（混合した2次のブロックが厳密に
   なりました。issue #192）。Hartree-Fock 項を使わない
   ``flex_second_order = "takimoto"``\ の計算は変わりません。\ **入力ファイルを変更する必要はありません。**

   **再現手順と複合パイプライン：**\ 2.0.0 の\ ``UHFk``\ の数値を再現するには、
   全てのオフサイト二体行の変位を反転してください
   （``[rx] [ry] [rz]`` → ``[-rx] [-ry] [-rz]``\ 。``Transfer``\ や\ ``Extern``\ などの一体項の行、
   およびオンサイトの行は対象外です。また、軌道の
