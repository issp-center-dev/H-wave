...
- ``flex_second_order``
  ...
  **説明 :**
  ...
  - ``"local"``\ （デフォルト）は厳密な **局所** 2次カーネルを構築します。
    ...
    オフサイト頂点が直接配置に入る全てのオンサイト・オフサイト混合図を
  ...
  - ``"takimoto"``\ は従来の Takimoto-Hotta-Ueda の式
    :math:`-\tfrac{1}{4}(\hat{U}^s + \hat{U}^c)\,\chi_0\,
    (\hat{U}^s + \hat{U}^c)`\ をそのまま保持します。H-wave 2.0.0 で
  ...
  **オンサイト同一軌道** の行（:math:`R = 0`\ 、
  :math:`\alpha = \beta`\ ）は起動時に拒否されます。このような行は二体項
  ではなく（一体のレベルシフト、実質的な\ ``CoulombIntra``\ 、あるいは
  恒等的にゼロ）です。エラーメッセージは該当行と、その種類に対する
