...
- ``flex_second_order``
  ...
  **計算コストとメモリ：** 本キーを導入したプルリクエストの1マップ計測スクリプト
  による実測（2軌道の\ ``tests/rpa/input_2orb``\ と合成した3軌道入力、
  :math:`L = 8`\ 、\ ``Nmat = 128``\ 、5回実行の最小値、2026-09）では、\ ``"local"``\ における\ :math:`V_{\rm eff}`\ の構築
  コストは従来の式の約 1.5 倍（2軌道で 1.59 倍、3軌道で 1.53 倍）、
  FLEX の1反復全体では約 1.2 倍（1.24 倍、1.16 倍）です。メモリは逆で、
