...
初期配位データファイルのファイル名 ``initial`` を指定します。指定がない場合は0で初期化されます。
...
乱雑位相近似RPAの入力ファイル形式に対応した StdFace ライブラリは以下から取得できます。
...
- ``calcmode = "uhfk"`` はH-wave向けに Wannier90(-like)形式での出力を指定します。 ``exportall = 0`` は出力をコンパクトにするオプションです。

入力ファイルの詳細については、 :ref:`Ch:HowToWannier90_rpa` のセクションを参照してください。
