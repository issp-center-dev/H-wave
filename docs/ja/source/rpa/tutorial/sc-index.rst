...
- ``chi0q_mode``: ``"load"`` はRPA出力ファイルから
  ...
  .. warning::
     ...
     **修正前の** ``calc_scheme = "general"`` **FLEX計算の出力を使う場合。**
     このパスは感受率を ``[a,c,b,d]`` ではなくMYOの軌道ペア順序で保存して
     いたため、``chi0q_mode = "flex"`` は転置されたペアリング頂点を構成し、
     同一の物理に対して ``"load"`` と食い違っていました。また自己エネルギーも
     軌道添字について転置されていたため、``bond_green`` でbondパスに渡す
     ``green.npz`` は軌道非対角成分が誤っています。いずれも修正済みです。
     修正前に生成した多軌道のgeneralパス出力は再生成してください。古い
     ``chiq_s``/``chiq_c`` は黙って誤読せず読み込み時に拒否されます。1軌道の
     計算および ``"reduced"``/``"squashed"`` スキームは影響を受けません。
