...
RAM の一部が上限に使われます。この見積もりは、設定の ``Nmat`` だけでなくディスク上の
ファイルヘッダ（``chiq_s.npz`` / ``chiq_c.npz`` / ``green.npz``\ ）から実際のグリッド
サイズを読み取ります。そのため、ファイルに保存された ``Nmat`` が設定と異なる場合は、
ロード途中で OOM で落ちるのではなく、確保前に事前に拒否されます。
