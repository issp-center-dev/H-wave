...
.. warning::
   ...
   **本バージョンでの変更。**
   ...
   何が変わるか。振動数パリティが定まった対振幅に対しては2つの処方は
   一致するため、パリティ射影を行った計算の先頭固有値も、固有値
   ソルバーが返す固有値も変わりません。以前の処方はカーネルと合成
   パリティとの可換性を破っており（奇振動数成分を偶セクターへ写して
   いました）、それにより実用上2つの帰結が生じていましたが、現在では
   いずれも解消されています。1つはべき乗法のパリティガードが
