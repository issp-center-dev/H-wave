...
``gap_dynamic.npz``
   ...
   - ``gap_sector_labels``: 上と同じ順序での 4 つのセクター名、すなわち
     ...
     ``"odd_k_odd_w"``\ 。
   - ``iteration_projection``: べき乗反復が各反復ベクトルを射影したセクター。
     ``"channel"``\ ・\ ``"combined_parity"``\ ・\ ``"none"``\ のいずれか
     （固有値ソルバーモードでは常に\ ``"none"``\ ）。同じ節を参照してください。
...
``match(1=channel-parity)``\ が付きます（指定セクターで\ ``1``\ 、セクター外で
...
警告され、``iteration_projection``\ として記録されます）。
...
.. note::
   ...
   **チャネルへの射影は、カーネルが振動数パリティを保存する場合にのみ適用されます**
   ...
   どちらの場合であったかは、``iteration_projection``\ （``"channel"``\ ・
   ``"combined_parity"``\ ・\ ``"none"``\ ）として npz のキーおよび固有値ファイルの
   ``# iteration_projection=...`` ヘッダー行に記録されます。固有値ソルバーモードは
