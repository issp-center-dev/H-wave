...
- ``num_eigenvalues``: 固有値モードで計算する固有値の数。
  ...
  として報告されるかは決めなくなりました。素の\ ``arnoldi``\ の集合に正の
  チャネル固有値が含まれない場合は実部最大の値を求めて一度だけ解き直すため
  （下記の\ ``spectral_shift``\ を参照）、主固有値は\ ``num_eigenvalues``\ に
  依存しません。
...
- ``spectral_shift``\ （\ ``eigenvalue_method = "arnoldi"``\ のときのみ有効）:
  ...
  *絶対値* より大きい正の\ :math:`\sigma`\ を与えます（\ :math:`A + \sigma I`\ の
  ...
  （\ ``frequency = "dynamic"``\ ）では、素の\ ``arnoldi``\ の集合に正のチャネル
  固有値が含まれない場合（主固有値が負、あるいは計算した固有対のいずれも要求
  チャネルに属さない場合）、ソルバーは自動的に\ ``spectral_shift = "auto"``\ で
  一度だけ解き直し、実部最大の固有値を代わりに報告します。その追加の求解は
  記録されます（\ ``eigenvalue_selection``\ 、下記参照）。あらかじめ
  ``spectral_shift = "auto"``\ を指定しておけば追加の求解は生じません。シード
  （固有ベクトル継続）付きの実行は解き直されないため、斥力優勢な温度スキャン
  では各点で\ ``spectral_shift = "auto"``\ を指定して、シード継続を主固有値と
  一致させてください。
...
``gap_dynamic.npz``
   ...
   - ``eigenvalue_selection``: 報告される主固有値を生んだ選択基準。``"LM"``
     （素の\ ``arnoldi``\ 、絶対値最大）・``"LR"``\ （利用者指定の
     ``spectral_shift``\ 、実部最大）・``"LR_retry"``\ （``"LM"``\ の集合に正の
     チャネル固有値が無かったときの、実部最大を求める自動再求解）・
     ``"shift-invert"``\ ・``"iteration"``\ （べき乗反復）のいずれかです。
     ``eigenvalue.dat``\ には\ ``# eigenvalue_selection: ...``\ ヘッダー行としても
     書き出されます。
...
.. note::
   ...
   **チャネルへの射影は、カーネルが振動数パリティを保存する場合にのみ適用されます**
   ...
   ``# sector_selection=...`` ヘッダー行に記録されます。固有値ソルバーモードは
   射影を行わず、計算された固有対を並べ替えるだけです。その際はチャネルの偶振動数
   セクターを優先し、そこに固有対が無い場合にのみ結合パリティセクターへ
   フォールバックします。この場合の\ ``sector_selection``\ は一致した段階を表し、
   ``eigenvalue.dat``\ の\ ``match``\ 列の見出しもそれに従います。

.. note::

   **主固有値は** ``num_eigenvalues`` **に依存しません。** 素の\ ``arnoldi``
   は ARPACK に *絶対値* 最大の固有値（\ ``which='LM'``\ ）を要求するため、より
   大きな斥力的（負）の固有値に埋もれて、小さな正（引力的）のチャネル固有値が
   短いリストから欠落することがあり、その場合に報告される主固有値は
   ``num_eigenvalues``\ に依存してしまいます。返された集合に正のチャネル固有値が
   無い場合、動的ソルバーは\ ``spectral_shift = "auto"``\ （実部最大）で\ **一度
   だけ**\ 解き直し、その結果を報告します。カーネルに引力的なモードが範囲内に
   本当に無ければ、答えは単に実部最大（最も斥力の弱い）固有値になります。用いた
   基準は\ ``eigenvalue_selection``\ として記録されます（``"LM"``\ ・``"LR"``
   （利用者指定の\ ``spectral_shift``\ ）・``"LR_retry"``\ （自動再求解）・
   ``"shift-invert"``\ ・``"iteration"``\ ）。npz のキーとしても、
   ``eigenvalue.dat``\ の\ ``# eigenvalue_selection: ...`` ヘッダー行としても
   書き出されます。あらかじめ\ ``spectral_shift = "auto"``\ を指定しておけば追加の
   求解は生じません。主となる正の固有値の基準は\ ``solver_mode = "iteration"``
