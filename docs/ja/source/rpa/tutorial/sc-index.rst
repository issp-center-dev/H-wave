...
``bond_cond_tol = 1e-4`` を必要とします。なお ``bond_cond_tol`` を
緩和しても、ガードを通過する点の計算済み固有値そのものは変わりません
（このしきい値は conditioning チェックのみをゲートし、線形連立方程式の
解法自体には影響しません）。実際には :math:`V = 1.2` において、この
準特異なウィンドウ端領域は生の動的固有値を **支配しています** -- 下記の
科学的な留保事項（``green0_tail`` ウィンドウ端アーティファクト、この点で
**86%** と測定）を参照してください。したがってこのガードを緩和することは、
絶対スケールがアーティファクトに支配されていると分かっている値のまま
先に進むという判断であり、その領域が無視できるという主張ではありません。
既存の ``bond_max_shells``、``bond_memory_cap_gb``、
