...
fft_workers`` は FFT のワーカースレッド数を指定します：``-1``（デフォルト）は全
コアを使用。複数の動的計算を同時に走らせる場合は、CPU の過剰割当てを避けるため
小さめの値（例：``OMP_NUM_THREADS`` に合わせる）に設定してください。両者を合わせ
