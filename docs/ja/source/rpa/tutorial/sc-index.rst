...
一重項ペアリング頂点は、スピンゆらぎ項・電荷ゆらぎ項・瞬間的な裸の項の和
...
であり、スピン（:math:`\chi_{\mathrm s}`）・電荷（:math:`\chi_{\mathrm c}`）
感受率について線形です。``[eliashberg]`` セクションの2つの任意のブール値により、
頂点を構成する前に一方のチャネルを 0 にでき、ペアリング強度をスピン／電荷ゆらぎに
切り分けられます。
...
**診断用** です：瞬間的な裸の :math:`\tfrac{1}{2}(\hat S + \hat C)` 項は常に
保持され、線形化ギャップ固有値問題は頂点について非線形なので、別々に 0 にした
計算の固有値は\ **加法的ではありません**\ （一般に
