...
.. warning::
   ...
   **多軌道の reduced/squashed FLEX を用いた計算結果は変化する可能性があります。**
   ...
   でした）。これは pairing vertex についての言明であって、両スキームが交換可能
   だという意味ではありません。自己無撞着に解いた場合、``reduced`` と
   ``general`` は自己エネルギーの構成も異なるため、``CoulombIntra`` のみでも
   収束結果は\ **一致しません**\ （2軌道モデルでの実測: 収束した自己エネルギー
   の差は約10%）。
...
.. note::
   ...
   **Eliashberg ステップは常磁性を前提とします。** Kuroki の :math:`S`/:math:`C`
   ...
   dressed Green 関数も同様に検査され、2ブロックの ``green.npz`` の
   スピンブロックが異なる場合は、黙って先頭を使わずに拒否します。その場合に得られる固有値は、スピン分解した
...
- **スピン混成を伴わない偏極**\ （``spin_mode = "spin-diag"``\ 。たとえば
  ...
  数値を返さずに例外を送出します。（保持ブロックに対して8桁以上小さい成分は
  丸め誤差として報告のうえ通過させるので、バックエンド由来の丸めの非対称で
  常磁性ランが止まることはありません。）

- **スピン混成・スピン軌道相互作用**\ （``mode.enable_spin_orbital = true``\ 、
  すなわち ``spin_mode = "spinful"``\ ）--- \ **サポートされていません。**
  FLEX 自体は動作しますが、``hwave_sc`` は形状エラーで停止します。FLEX は
  物理軌道数で chi を書く一方、Eliashberg 側は ``geom.dat`` の ``norb``\ ---
  スピン軌道モードではスピン軌道数---を読むためです。エラーメッセージが
  この場合であることを明示します。
