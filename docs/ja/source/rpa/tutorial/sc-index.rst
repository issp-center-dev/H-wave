...
- ``singlet``: :math:`\mathbf{k}` について偶、振動数について偶（2 つの軌道
  添字は転置されます）。
...
.. note::
   ...
   運動量パリティと振動数パリティは、単一軌道模型では常に、また対応する格子・軌道
   対称性を持つ多軌道模型でも、ペアリングカーネルによって別々に保存されます。
   一般の多軌道カーネルはそれらの積（反対称性の制約）のみを保存するため、偶振動数
   チャネルは不変部分空間ではなく固有値問題の *制限* となり、報告される
   :math:`\lambda` はその部分空間に制限したカーネルの最大固有値になります。
   記録されるセクター重みは、この違いを可視化するためのものです。
