...
.. warning::
   ...
   **本バージョンでの変更——該当する計算は再実行してください。**
   ...
   なりました。以前のバージョンで用いていた片側の値
   :math:`F(0^+)`\ ではありません。

   これは通常の（オンサイトの）動的ソルバーの結果を変える変更であり、
   ボンド分解バーテックスに固有の話ではありません。瞬時部分が非零の
   ``matsubara_basis = "ir"``\ 計算すべてに適用されます。瞬時部分は
   オフサイト相互作用（係数が非零の\ ``CoulombInter`` / ``Hund`` /
   ``Ising``\ ）が宣言されていれば直ちに非零になります。オンサイトのみの
   模型は瞬時部分を持たないため影響を受けず、一様グリッドの経路
   （``matsubara_basis = "uniform"``\ ）はいずれの場合も影響を受けません。

   これは規約の選択ではなく修正です。片側の値はカーネルのパリティ
   可換性を破り、``Nmat``\ を増やしても一様グリッドの結果に収束しません
   でしたが、中間点はそのいずれも満たします。該当する模型について
   以前のバージョンで得た動的 IR の結果がある場合は、再計算して
   ください。
