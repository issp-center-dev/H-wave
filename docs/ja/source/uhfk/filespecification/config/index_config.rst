...
- ``BoundaryCondition``
  ...
  **説明 :**
  ...
  任意の ``SubShape`` (デフォルトの ``SubShape = CellShape`` を含む) と
  併用可能です。ゲージ位相は ``Transfer`` の原始(符号付き変位)表現に対して
  副格子フォールド前に適用されるため、 ``SubShape`` の選択は物理結果に影響しません。

  非密度型相互作用 ( ``PairHop`` , ``PairLift`` ) と反周期境界条件の併用は
  サイト依存ゲージ下でゲージ不変ではないため、例外として拒絶されます。
