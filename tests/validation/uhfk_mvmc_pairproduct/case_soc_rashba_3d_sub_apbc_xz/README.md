# case_soc_rashba_3d_sub_apbc_xz (v3.7 shipping fixture)

xy-plane Rashba SOC + general complex z-direction spin-mixing hopping
fixture with antiperiodic boundary condition in x and z (xz plane
APBC, y periodic). Common lattice across all 4 v3.7 shipping fixtures.

## Geometry / interaction

- `CellShape` = `[4, 4, 4]`
- `SubShape` = `[2, 2, 2]` (folded BZ `L_folded = [2, 2, 2]`)
- `BoundaryCondition` = `["antiperiodic", "periodic", "antiperiodic"]` (2D APBC in xz plane)
...
`BoundaryCondition` (theta = (pi, 0, pi)):

```
PYTHONPATH=src python tests/validation/uhfk_mvmc_pairproduct/scripts/soc_pilot_gap.py \
    --case tests/validation/uhfk_mvmc_pairproduct/case_soc_rashba_3d_sub_apbc_xz \
...
- `Ncond= 12`: gap = 1.776e-15 (FAIL; degenerate HOMO/LUMO)
- `Ncond= 14`: gap = 2.665e-15 (FAIL; degenerate HOMO/LUMO)
- `Ncond= 16`: gap = 6.551e-02 (PASS); partner-balance FAIL (k=4/partner=1: `2 != 3`)
- `Ncond= 18`: gap = 2.173e-01 (PASS); partner-balance FAIL (k=4/partner=1: `2 != 4`)
- `Ncond= 20`: gap = 2.173e-01 (PASS); partner-balance **PASS**
- `Ncond= 22`: gap = 2.530e-02 (FAIL; below the 5e-2 floor); partner-balance PASS
- `Ncond= 24`: gap = 6.551e-02 (PASS); partner-balance **PASS**
...
24}`. `Ncond = 20` stays pinned: it has the larger gap (2.173e-01 vs
6.551e-02), it matches the xy sibling's pinned filling, and it sits
inside spec §1.2's "expected range 16-24 for closed-shell filling".
The switch to `flag_fock = true` therefore does not move this
fixture's filling.

Pinned: `Ncond = 20` (gap = 2.173e-01 >= 5e-2, partner-balance holds).

## v3.7 gate role

Phase 6 seven-gate run: `bash tests/validation/uhfk_mvmc_pairproduct/run.sh case_soc_rashba_3d_sub_apbc_xz` produces the G0-writer-check + G1 + G2a-emitted-F + G2a-in-memory-A + G2b + G3 + G4 records at 1e-6 tolerance for the SOC + APBC(xz) + SubShape[2,2,2] triple validation.

The regenerated G4 manifest keeps the 30-entry per-direction schema
and selects composite `i=0 s=1 j=63 t=0`,
`|G_c|=2.8631e-02`. Its active x/z axes contribute 20 distinct,
positive-threshold mutation evaluations; the 10 y-axis entries remain
