...
class RPATwoOrbital:
    ...
    def _get_epsilon(self, kx_array, ky_array, t):
        ...
        for idx, kx in enumerate(kx_array):
            for idy, ky in enumerate(ky_array):
                ...
                epsilon_k[0][1][idx][idy] = t*(1.0+np.exp(1J*kx))
                epsilon_k[1][0][idx][idy] = t*(1.0+np.exp(-1J*kx))
        return epsilon_k
    ...
    def run(self, params):
        ...
        myu   = params.get("myu", 0.0)
        beta  = params.get("beta", 1.0)  #XXX
        norb  = 2

        kx_array = np.linspace(0, 2.*np.pi, Lx, endpoint=False)
        ky_array = np.linspace(0, 2.*np.pi, Ly, endpoint=False)

        epsilon_k = self._get_epsilon(kx_array, ky_array, t)
            ...
            for jorb in range(2):
                for iorb1 in range(2):
                    for jorb1 in range(2):
                        for igamma in range(2):
                            chi0q[iorb][iorb1][jorb][jorb1] += self._calc_chi0q(Lx, Ly, Nmat, green_kw[igamma][iorb][jorb], green_kw[igamma][jorb1][iorb1], myu, beta)
...
class TestRPATwoOrbital(unittest.TestCase):
    def run_test(self, params, params_ref):
        ...
        print(solver.ham_info.ham_inter_q.shape)
        ham = solver.ham_info.ham_inter_q.reshape(Lx,Ly,16,16)
        ...
        chi0q_ref, chiq_ref, ham_ref, kxarr, kyarr = rpatwo.run(params_ref)
