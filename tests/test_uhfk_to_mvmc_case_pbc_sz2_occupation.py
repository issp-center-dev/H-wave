"""Wrap `scripts/assert_occupation.py case_pbc_sz2` in a pytest so CI can
guard the fixture's target occupation before the mVMC harness runs."""
...
CASE_DIR = os.path.join(
    REPO_ROOT, "tests/validation/uhfk_mvmc_pairproduct/case_pbc_sz2"
)
...
def _run_hwave(work_dir):
    ...
    if res.returncode != 0:
        pytest.skip(f"H-wave SCF failed for case_pbc_sz2: {res.stderr}")


def test_case_pbc_sz2_target_occupation_holds():
