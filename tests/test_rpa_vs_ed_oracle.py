...
"""Exact diagonalization as the ground truth for every solver mode.
...
class TestRPAGeneralVsED(unittest.TestCase):
    """Every declaration type, general scheme, against exact
    diagonalization: the solver must reproduce the exact first-order
    vertex response on the slots the ring form can populate, and must
    stay silent exactly on the exchange-only slots."""
    ...
    def test_pairlift_content_is_outside_the_longitudinal_ring(self):
        ...
        spin-diagonally); its transverse counterpart is what represents
        this channel. The solver is therefore silent here, and the whole
        exact response must sit in spin-flip slots -- recorded so a
        future change that starts populating them is noticed."""
