"""Golden-file regression: the DEFAULT static-Eliashberg path must still
produce the pre-branch bytes.
...
jitter, issue #85), everything downstream of it is.
...
def test_default_run_is_byte_identical_to_712b1a0(tmp_path):
    """No ``bond_channels``, no ``spectral_shift`` -> exactly the 712b1a0
    bytes, comment lines included."""
    got = _run_legacy(str(tmp_path / "run"))
    assert got["eigenvalue.dat"] == _golden("eigenvalue.dat")
    assert got["gap.dat"] == _golden("gap.dat")


def test_bond_channels_false_is_byte_identical_to_712b1a0(tmp_path):
    """Explicitly disabling the new flag must also reproduce pre-branch
    bytes (not merely agree with the flag-absent run of the new build)."""
