...
- ``longitudinal_bond_guard_freqs``
  ...
  **説明 :**
  ...
  （旧バージョンの挙動）。GPU 経路（\ ``gpu = true``\ ）では、\
  ``"all"``\ はもはや全てのブロックをホストで分解しません。各ブロックの
  条件数について、デバイス上でバッチ計算した厳密な両側バウンド（行列
  ノルム、残差付きの近似逆行列、数回のべき乗反復）を用い、バッチ最小値
  になり得るブロックのみを厳密に分解します。そのためガードの出力
  （最小値、その位置、拒否の判定）は全ブロック分解の場合と完全に一致
  しつつ、\ ``"all"``\ のコストはおよそ半減します（3 バンドの CuO2
  モデル、8x8、Nmat 2048 で A100 上で測定：以前は 1 反復あたり約 55 秒、
  現在は 23-31 秒、厳密分解されるブロックは 30-50 %\ 。\ ``"static"``\
  は約 9 秒）。出力には\ ``longitudinal_bond_guard_method``\ （\
  ``"svd"``\ / \ ``"interval"``\ ）と厳密分解のカウントが記録されます。
  CPU 経路は変更されません（\ ``"svd"``\ ）。\ ``"static"``\ はゼロ振動数のみを特異値分解で
  検査し、その他のスライスは解の残差
