...
アーカイブには\ ``[mode.param] longitudinal_bond_output_layout``\ で選択した
ディスク上のレイアウトを表す\ ``bond_archive_layout``\ （文字列）も記録されます。
デフォルトの\ ``"npz"``\ レイアウトでは\ ``chi_s_w``\ と\ ``chi_c_w``\ はこの単一
ファイル内に格納されます。\ ``"sidecar"``\ レイアウトでは、2つのチャネルメンバーは
代わりにインデックスの隣に素の\ ``.npy``\ ファイル（``<name>_chi_s_w.npy``\ および
``<name>_chi_c_w.npy``\ ）として書き出され、このファイルからは省かれます。
その代わりに\ ``chi_s_w_file``\ と\ ``chi_c_w_file``\ （文字列。sidecar の
ベース名で、インデックスのあるディレクトリからの相対で解決されます）が
格納されます。後処理はこれらの\ ``.npy``\ ファイルをメモリマップし、振動数
バッチを1つずつ読み込むため、大きなチャネルメンバー全体がメモリに常駐する
ことはありません。\ ``bond_archive_layout``\ メンバーを持たないアーカイブ
（このフィールドが存在する前に書き出されたもの）は\ ``"npz"``\ として読み込まれます。
